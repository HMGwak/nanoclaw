...
- **배점**: 0
- **하드 게이트**: 3
- **설명**: 변경제출 절차/단계가 명시된 섹션 수

#### 측정 방법
```python
import re
def measure(output_files, reference_files):
    STEP_KEYWORDS = ["접수", "요청", "변경", "자료", "제출", "송부", "확인", "절차", "국가", "서류", "유형"]
    for f in output_files:
        text = f.read_text()
...
- **배점**: 25
- **하드 게이트**: 없음
- **설명**: 변경제출의 핵심 측면(변경점 유형, 국가별 요건, 절차, 필수 서류, 이슈)을 얼마나 포괄하는지
...
- **배점**: 20
- **하드 게이트**: 없음
- **설명**: 신규 담당자가 이 wiki note만 보고 변경제출 업무를 수행할 수 있는지
