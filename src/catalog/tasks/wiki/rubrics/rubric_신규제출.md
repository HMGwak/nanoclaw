...
- **배점**: 0
- **하드 게이트**: 3
- **설명**: 제출 절차/단계가 명시된 섹션 수

#### 측정 방법
```python
import re
def measure(output_files, reference_files):
    STEP_KEYWORDS = ["접수", "요청", "자료", "수령", "추출", "작성", "제출", "송부", "확인", "절차", "국가", "제품", "초안"]
    for f in output_files:
        text = f.read_text()
...
- **배점**: 25
- **하드 게이트**: 없음
- **설명**: 신규제출의 핵심 측면(국가별 요건, 제출 절차, 필수 서류, 첨가물 추출, 이슈)을 얼마나 포괄하는지
...
- **배점**: 20
- **하드 게이트**: 없음
- **설명**: 신규 담당자가 이 wiki note만 보고 신규제출 업무를 수행할 수 있는지
